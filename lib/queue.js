...
Broker.prototype.push = function(options, next) {
  ...
  self._getQueue(options.queue, function (err, qurl) {
    ...
    if (options.unique) {
      if (!self.cache)
        return next(new Error("Unique pushes requires cache instance."));

      if (!options.delay)
        return next(new Error("Unique pushes requires delay."));

      self.cache.add("awscp:queue:" + options.queue + ":unq:" + options.unique, 1, options.delay, function (err) {
        if (err && err.notStored) return next();
        if (err) return next(err);
        _push();
      })
    } else {
